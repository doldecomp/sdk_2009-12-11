...
#define DARCH_SIGNATURE	0x55aa382d
...
struct DARCHItem
{
	...
	int			childCount;	// size 0x04, offset 0x20
}; // size 0x24
                                      ...
                                      void *dstStartBuffer, u32 dstSize)
{
	...
	items->childCount = fileInfoCount;
	it = items;
	fileCount = 0;
	dataSize = stringPoolSize + fileInfoCount * sizeof(DARCHFileInfo);
	itemOffset = sizeof(DARCHHeader);
	fullSize = ROUND_UP(itemOffset + dataSize, 32);
	data = (char *)dstStartBuffer + itemOffset;
	end = start = data + fileInfoCount * sizeof(DARCHFileInfo);
	tmpSize = fullSize;
	...
	for (i = 0; i < fileInfoCount; ++i)
	{
		...
		for (int j = 0; j < 1; ++j)
			0[data + j] = it->isDir >> ((1 - j - 1) * 8) & 0xff; // TODO: What

		for (int j = 0; j < 3; ++j)
			1[data + j] = (end - start) >> ((2 - j) * 8) & 0xff;
		...
		{
			for (int j = 0; j < 4; ++j)
				4[data + j] = it->parentDir->id >> ((3 - j) * 8) & 0xff;

			for (int j = 0; j < 4; ++j)
				8[data + j] = (i + it->childCount) >> ((3 - j) * 8) & 0xff;
		}
		else
		{
			for (int j = 0; j < 4; ++j)
				4[data + j] = tmpSize >> ((3 - j) * 8) & 0xff;

			for (int j = 0; j < 4; ++j)
			{
				8[data + j] =
					fileInfos[fileCount].fileSize >> ((3 - j) * 8) & 0xff;
			}
...
u32 DARCHGetArcSize(DARCHFileInfo *fileInfos, u32 fileInfoCount)
{
	...
	u32 dataSize =
		ROUND_DOWN(63 + itemCount * sizeof(DARCHFileInfo) + stringPoolLen, 32);
	infoSize = dataSize;
