...
void btm_discovery_db_init(void)
{
	...
	btm_cb.btm_disc_vars.uuid_filters[0].uu.uuid16 = 0x1002;

	SDP_InitDiscoveryDb(btm_cb.p_btm_ddb,
	                    sizeof btm_cb.btm_disc_vars.btm_db_area, 1,
	                    &btm_cb.btm_disc_vars.uuid_filters[0], 0, NULL);
}
...
void btm_discovery_db_reset(void)
{
	...
	{
		btm_cb.btm_disc_vars.disc_active = 0;
...
static void btm_disc_compl_cb(UINT16 result)
{
	...
	if (result != 0xfff4 && btm_cb.btm_disc_vars.disc_active != 2)
	{
...
tBTM_STATUS BTM_StartDiscovery(tBTM_CMPL_CB *p_cmpl_cb, BD_ADDR p_rem_addr)
{
	...
	if (btm_cb.btm_disc_vars.disc_active != 0)
		return 2;
	...
	SDP_InitDiscoveryDb(btm_cb.p_btm_ddb,
	                    sizeof btm_cb.btm_disc_vars.btm_db_area,
