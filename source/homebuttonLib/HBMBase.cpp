...
// [SC5PGN]/build/libs/Debug/slamWiiD.a:HBMBase.o(1)::.debug_info::0x728 [original object]
...
namespace homebutton
{
	...
	static void drawBlackPlate(f32 left, f32 top, f32 right, f32 bottom,
	                           GXColor clr);
	static void drawTexPlate(void *graphicBuf, u16 width, u16 height,
	                         GXTexFmt texFmt, f32 left, f32 top, f32 right,
	                         f32 bottom, f32 z, GXColor clr);
...
static void initgxfortex() ATTR_UNUSED
{
	GXClearVtxDesc();
	GXSetVtxDesc(GX_VA_POS, GX_DIRECT);
...
{
	GXTexObj texObj;
	GXInitTexObj(&texObj, graphicBuf, (u16)width, (u16)height, texFmt, GX_CLAMP,
	             GX_CLAMP, GX_FALSE);
	GXInitTexObjLOD(&texObj, GX_NEAR, GX_NEAR, 0.0f, 0.0f, 0.0f, GX_FALSE,
	                GX_FALSE, GX_ANISO_1);
	GXInitTexObjFilter(&texObj, 5, 1);
...
{
	byte4_t data = OSGetAlarmUserDataAny(byte4_t, alm);
	int chan = (data >> 16) & 0xffff;
	int id = data & 0xffff;

	HomeButton *pHBObj = HomeButton::getInstance();
	OSAssert_Line(1978, pHBObj);
...
static void SimpleSyncCallback(s32 result, s32 num)
{
	OSAssert_Line(2045, HomeButton::getInstance());
	...
	if (isActive() && btn_no != -1)
	{
		if (mSequence == eSeq_Normal && btn_no < mButtonNum)
		{
			...
			if (mDialogFlag[btn_no])
			{
				...
				OSAssert_Line(3009, p_pane);

				nw4hbm::lyt::TextBox *p_text =
					nw4hbm::ut::DynamicCast<nw4hbm::lyt::TextBox *>(p_pane);

				OSAssert_Line(3011, p_text);
...
int HomeButton::getPaneNo(nw4hbm::lyt::Pane const *pPane)
{
	...
	for (int i = 0; i < (int)ARRAY_LENGTH(scFuncTouchPaneName); ++i)
	{
