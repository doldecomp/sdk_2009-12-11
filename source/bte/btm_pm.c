                             ...
                             tBTM_PM_PWR_MD *p_mode)
{
	...
	if (p_cb->state == BTM_PM_STS_HOLD || p_cb->state == 4
                             ...
                             UINT16 interval)
{
	...
	if (mode == 0 && p->restore_pkt_types && btm_num_sco_links_active() == 0)
	{
	...
	if (btm_cb.acl_db[xx].switch_role_state != 1)
		return;
	...
	{
		if (btsnd_hcic_switch_role(btm_cb.acl_db[xx].remote_addr,
	                                !btm_cb.acl_db[xx].link_role))
		{
			btm_cb.acl_db[xx].switch_role_state = 0;
			return;
		}
	}
